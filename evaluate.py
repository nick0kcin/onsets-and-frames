...
def evaluate(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None):
    ...
    for label in data:
        for key in label:
            label[key] = label[key].to(DEFAULT_DEVICE)
        ...
        p_ref, i_ref, v_ref = extract_notes(label['onset'], label['frame'])
        p_est, i_est, v_est = extract_notes(pred['onset'], pred['frame'], onset_threshold=onset_threshold, frame_threshold=frame_threshold)
